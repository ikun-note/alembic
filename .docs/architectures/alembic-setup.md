...
- 后端: FastAPI + SQLAlchemy 2.x + Alembic + psycopg3; 配置统一走 `.env` (密钥 + 非密钥)
...
| `backend/app/core/config` | 合并 DEFAULT + ENV (.env), 输出单一 Settings | 不执行迁移、不处理 HTTP |
...
2. `Settings` 读取 DEFAULT + `.env`, 校验汇总为单一对象
...
- 密钥不进配置文件: 密码只在 `.env`, 不进 `alembic.ini` 与代码
- 单一配置对象: DEFAULT + ENV (.env) 汇总到 Settings, 应用与 env.py 共用同一来源
- 配置分层优先级: CLI > ENV > DEFAULT (应用配置在 `.env`; `alembic.ini` 仍是 Alembic 自身的非密钥配置)
...
| 配置形态 | 仅 `.env` (无 config 文件) | 本项目配置仅 DB 连接, 单文件最简; 有大量非密钥结构化配置时再引入 config 文件 |
...
| 生产降级策略 | 只进不退 (roll forward) | 结构降级会永久丢数据; 出问题写修复迁移而非 `downgrade`, 降级仅限开发 / 测试 |
